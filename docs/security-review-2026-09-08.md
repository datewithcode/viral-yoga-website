...
| F03 | Medium | Enquiry limits were checked and inserted separately, so parallel requests bypassed them; the global limit let one bot block everyone. | Limits moved into one database function `submit_enquiry` that takes an advisory lock per phone and per IP: 3 per phone, 5 per IP address, 100 in total per 10 minutes as a circuit breaker only. Callable by `service_role` only. | "parallel burst: exactly three stored", "fourth from one phone refused", "sixth from one address refused", "student cannot call submit_enquiry directly" |
